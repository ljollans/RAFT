function [params2pick, Merit, Beta, Vars2pick_main,  GetProbs, design, stats, params2use] = RAFT_Model_Selection(design, use_pass_vars,reg_on)
...
if strcmp(design.type, 'logistic')==1 && exist('reg_on', 'var')==0 | isempty(reg_on)
    truth=design.outcome;
...
for mainfold=1:design.numFolds
    ...
    [params2pick(:,mainfold) paramidx paramidx_perfold lambdavalues] = findbestparams(design, mainfold,reg_on);
...
%             [r p]=corr(design.outcome(testsubjects), GetProbs{mainfold});
            ...
            [Merit.accuracy(mainfold) Merit.sensitivity(mainfold) Merit.specificity(mainfold) Merit.AUC(mainfold) Merit.F1(mainfold)]=class_mets(logical(design.outcome(testsubjects)), GetProbs{mainfold}');
    end
end
    cd(design.saveto);
    clear stats
    switch(design.type)
        ...
        case 'logistic'
            ...
            [stats.accuracy stats.sensitivity stats.specificity stats.AUC stats.F1]=class_mets(logical(design.outcome), design.prediction);
            save('Results',  'Merit', 'GetProbs', 'Beta', 'params2pick', 'params2use', 'stats', 'Vars2pick_main', 'design');
    end
end

function [optim_mainfold paramidx_persubfold subfoldidx lambdamatrix] = findbestparams(design, mainfold, reg_on)

if exist([design.saveto filesep 'LHmerit.mat'])~=0
    load([design.saveto filesep 'LHmerit.mat']);
    eval(['LHmerit' num2str(mainfold) '=squeeze(LHmerit.' reg_on '(mainfold, :,:,:,:,:));']);
else
load([design.saveto filesep 'LHmerit' num2str(mainfold)]);
eval(['LHmerit' num2str(mainfold) '=LHmerit' num2str(mainfold) '.' reg_on ';']);
end
