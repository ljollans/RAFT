function RAFT_2nd_Level_180717(design, pass_vars, folds2run)
...
parfor folds=1:(design.numFolds*design.numFolds)
    if isempty(find(folds==folds2run))==0;
        ...
        tmpLHmerit=NaN(design.numFolds, size(design.merit_thresholds,2),design.numAlphas, design.numLambdas);
            case 'logistic'
                tmpLHmerit.accuracy=NaN(design.numFolds, size(design.merit_thresholds,2),design.numAlphas, design.numLambdas);
                tmpLHmerit.sensitivity=NaN(design.numFolds, size(design.merit_thresholds,2),design.numAlphas, design.numLambdas);
                tmpLHmerit.specificity=NaN(design.numFolds, size(design.merit_thresholds,2),design.numAlphas, design.numLambdas);
                tmpLHmerit.overall_AUC=NaN(design.numFolds, size(design.merit_thresholds,2),design.numAlphas, design.numLambdas);
                tmpLHmerit.F1score=NaN(design.numFolds, size(design.merit_thresholds,2),design.numAlphas, design.numLambdas);
        end
            ...
            for alpha_looper=1:length(design.alpha)
                ...
                if design.nboot>1
                     ...
                     switch(design.type)
                            ...
                            case 'logistic',
                                tmpmerit.accuracy=NaN(design.nboot,design.numLambdas);
                                tmpmerit.sensitivity=NaN(design.nboot,design.numLambdas);
                                tmpmerit.specificity=NaN(design.nboot,design.numLambdas);
                                tmpmerit.overall_AUC=NaN(design.nboot,design.numLambdas);
                                tmpmerit.F1score=NaN(design.nboot,design.numLambdas);
                        end
                    ...
                    for bootct=1:design.nboot
                        ...
                        switch(design.type)
                            ...
                            case 'logistic',
                                merit.accuracy=NaN(1,design.numLambdas);
                                merit.sensitivity=NaN(1,design.numLambdas);
                                merit.specificity=NaN(1,design.numLambdas);
                                merit.overall_AUC=NaN(1,design.numLambdas);
                                merit.F1score=NaN(1,design.numLambdas);
                        end
                        ...
                        for lambda_looper=1:length(design.lambda)
                            ...
                            switch(design.type)
                                ...
                                case 'logistic',
                                    ...
                                    [merit.accuracy(lambda_looper) merit.sensitivity(lambda_looper) merit.specificity(lambda_looper) merit.overall_AUC(lambda_looper) merit.F1score(lambda_looper)]=class_mets(design.outcome(testsubjectsTune), getprobstune');
                                    % end
                            end
                        end
                        ...
                        switch(design.type)
                            case 'logistic'
                                tmpmerit.accuracy(bootct,:)=merit.accuracy;
                                tmpmerit.sensitivity(bootct,:)=merit.sensitivity;
                                tmpmerit.specificity(bootct,:)=merit.specificity;
                                tmpmerit.overall_AUC(bootct,:)=merit.overall_AUC;
                                tmpmerit.F1score(bootct,:)=merit.F1score;
                            case 'linear'
                    ...
                    for lambda_looper=1:length(design.lambda)
                        ...
                        if strcmp(design.bagcrit, 'median')==1
                            switch(design.type)
                                case 'linear',
                                ...
                                case 'logistic',
                                    tmpLHmerit.accuracy(ROIcrit, meritthresh_looper, alpha_looper,lambda_looper)=median(tmpmerit.accuracy(:,lambda_looper));
                                    tmpLHmerit.sensitivity(ROIcrit, meritthresh_looper, alpha_looper,lambda_looper)=median(tmpmerit.sensitivity(:,lambda_looper));
                                    tmpLHmerit.specificity(ROIcrit, meritthresh_looper, alpha_looper,lambda_looper)=median(tmpmerit.specificity(:,lambda_looper));
                                    tmpLHmerit.overall_AUC(ROIcrit, meritthresh_looper, alpha_looper,lambda_looper)=median(tmpmerit.overall_AUC(:,lambda_looper));
                                    tmpLHmerit.F1score(ROIcrit, meritthresh_looper, alpha_looper,lambda_looper)=median(tmpmerit.F1score(:,lambda_looper));
                            end
                        else
                            disp('Please enter a valid method for bagging (median ONLY at the moment)')
                        end
                    ...
                    switch(design.type)
                        ...
                        case 'logistic',
                            tmpLHmerit.accuracy(ROIcrit, meritthresh_looper, alpha_looper,:)=merit.accuracy;
                            tmpLHmerit.sensitivity(ROIcrit, meritthresh_looper, alpha_looper,:)=merit.sensitivity;
                            tmpLHmerit.specificity(ROIcrit, meritthresh_looper, alpha_looper,:)=merit.specificity;
                            tmpLHmerit.overall_AUC(ROIcrit, meritthresh_looper, alpha_looper,:)=merit.overall_AUC;
                            tmpLHmerit.F1score(ROIcrit, meritthresh_looper, alpha_looper,:)=merit.F1score;
                    end
        ...
        for filler=1:length(tofillin)
            ...
            switch(design.type)
                ...
                case 'logistic'
                    tmpLHmerit.accuracy(ROIcrit, meritthresh_looper, :,:)=tmpLHmerit.accuracy(ROI_duplicate_match(filler), merit_duplicate_match(filler), :,:);
                    tmpLHmerit.sensitivity(ROIcrit, meritthresh_looper, :,:)=tmpLHmerit.sensitivity(ROI_duplicate_match(filler), merit_duplicate_match(filler), :,:);
                    tmpLHmerit.specificity(ROIcrit, meritthresh_looper, :,:)=tmpLHmerit.specificity(ROI_duplicate_match(filler), merit_duplicate_match(filler), :,:);
                    tmpLHmerit.overall_AUC(ROIcrit, meritthresh_looper, :,:)=tmpLHmerit.overall_AUC(ROI_duplicate_match(filler), merit_duplicate_match(filler), :,:);
                    tmpLHmerit.F1score(ROIcrit, meritthresh_looper, :,:)=tmpLHmerit.F1score(ROI_duplicate_match(filler), merit_duplicate_match(filler), :,:);
            end
